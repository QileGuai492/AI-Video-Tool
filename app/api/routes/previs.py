...
@router.post("/generate-from-video-advanced", response_model=PrevisProjectRead)
async def generate_previs_project_from_video_advanced(
    file: UploadFile = File(...),
    prompt: str | None = Form(None),
    title: str | None = Form(None),
    db: Session = Depends(get_db),
